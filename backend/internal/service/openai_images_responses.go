...
func shouldFallbackOpenAIImagesOAuthToDirect(statusCode int, upstreamMsg string, body []byte) bool {
...
func (s *OpenAIGatewayService) forwardOpenAIImagesOAuth(
	...
	if resp.StatusCode >= 400 {
		...
		if shouldFallbackOpenAIImagesOAuthToDirect(resp.StatusCode, upstreamMsg, respBody) {
			// OAuth session tokens are also valid on the public OpenAI API.
			// When the Codex/Responses API returns a model-support error for
			// ChatGPT accounts, fall back to the direct Images API.
			logger.LegacyPrintf(
				"service.openai_gateway",
